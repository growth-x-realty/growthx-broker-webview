...
type leadStore = {
    data: Map<string, Omit<Lead, "b_id">[]>; // p_id -> Lead[]
    keys: Omit<Lead, "b_id">[]; // Lead[]
    setLeads: (leads: Omit<Lead, "b_id">[]) => void;
    addLead: (lead: Omit<Lead, "b_id">) => void;
}
export const useStoreLead = create<leadStore>((set) => ({
    ...
    setLeads: (leads) => {
        set(() => {
            const data = new Map();
            const keys: Omit<Lead, "b_id">[] = [];
            leads.forEach((lead) => {
                data.set(lead.p_id, [lead, ...data.get(lead.p_id) || []]);
                keys.push(lead);
            })
